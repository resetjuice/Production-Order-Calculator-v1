...
<head>
    ...
    <title>Calculadora v4.2 - Reset Juice</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        <div class="content">
            ...
            <div id="results" class="results">
                ...
                <div class="result-card result-main">
                    <h4>🎯 CANTIDAD FINAL RECOMENDADA</h4>
                    <div class="result-value" id="recommendedOrder">--</div>
                    <p id="finalOrderDescription">Con normalización estacional v4.2</p>
                </div>
                
                <div class="comparison-table" id="comparisonTable" style="display: none;">
                    <h5>⚖️ Comparación: Sin Normalizar vs CON Normalización</h5>
                    <table>
                        <thead>
                            <tr>
                                <th>Método</th>
                ...
                <div class="result-card">
                    <h4>Stock de Seguridad</h4>
                    <div class="result-value" id="safetyStock">--</div>
                    <p>Basado en volatilidad del producto</p>
                </div>
                ...
                <div class="result-card">
                    ...
                    <p>Proyección basada en demanda normalizada</p>
                </div>
                ...
                <div class="calculation-details">
                    <h4>Cálculos Paso a Paso</h4>
                    <div id="calculationSteps">--</div>
                </div>
            </div>
            
            <div class="methodology">
                <h4>Metodología v4.2: Normalización Estacional</h4>
                <ul>
                    <li><strong>Normalización Estacional:</strong> Cada mes histórico se divide por su factor estacional específico</li>
                    <li><strong>Eliminación de Sesgos:</strong> Los datos de temporada baja no contaminan predicciones de temporada alta</li>
                    <li><strong>Promedio Ponderado:</strong> Mayor peso al mes más reciente (50% vs 30% vs 20%)</li>
                    <li><strong>Control de Vida Útil:</strong> Nunca recomienda más de lo vendible en días de caducidad</li>
                    <li><strong>Ajuste por Stock:</strong> Descuenta automáticamente el inventario disponible</li>
                    <li><strong>Personalización Total:</strong> Cada producto mantiene sus patrones únicos</li>
                </ul>
            </div>
        </div>
    </div>
...
const productDatabase = {
    'carrot-rush-480': {
        name: 'Carrot Rush 480ml', shelfLife: 75,
        seasonalIndices: {
    ...
    'greenergy-480': {
        name: 'Greenergy 480ml', shelfLife: 80,
        seasonalIndices: {
    ...
    'greenergy-350': {
        name: 'Greenergy 350ml', shelfLife: 100,
        seasonalIndices: {
    ...
    'unbeetable-480': {
        name: 'Unbeetable 480ml', shelfLife: 80,
        seasonalIndices: {
    ...
    'unbeetable-350': {
        name: 'Unbeetable 350ml', shelfLife: 100,
        seasonalIndices: {
    ...
    'pinebliss-480': {
        name: 'Pinebliss 480ml', shelfLife: 120,
        seasonalIndices: {
    ...
    'pinebliss-350': {
        name: 'Pinebliss 350ml', shelfLife: 120,
        seasonalIndices: {
    ...
    'vitality-mylk-480': {
        name: 'Vitality Mylk 480ml', shelfLife: 20,
        seasonalIndices: {
    ...
    'coco-dream-480': {
        name: 'Coco Dream 480ml', shelfLife: 20,
        seasonalIndices: {
    ...
    'chokonut-480': {
        name: 'Chokonut 480ml', shelfLife: 25,
        seasonalIndices: {
    ...
    'coco-blast-480': {
        name: 'Coco Blast 480ml', shelfLife: 65,
        seasonalIndices: {
    ...
    'spicy-limonata-480': {
        name: 'Spicy Limonata 480ml', shelfLife: 120,
        seasonalIndices: {
    ...
    'c-master-480': {
        name: 'C-Master 480ml', shelfLife: 70,
        seasonalIndices: {
    ...
    'black-magic-480': {
        name: 'Black Magic 480ml', shelfLife: 135,
        seasonalIndices: {
    ...
    'tropik-glow-480': {
        name: 'Tropik Glow 480ml', shelfLife: 70,
        seasonalIndices: {
    ...
    'gingerlicious-50': {
        name: 'Gingerlicious 50ml', shelfLife: 150,
        seasonalIndices: {
    ...
    'gingerlicious-250': {
        name: 'Gingerlicious 250ml', shelfLife: 120,
        seasonalIndices: {
...
function analyzeProduct() {
    ...
    if (!product) {
        productAlert.innerHTML = '<div class="product-alert alert-danger">❌ Producto no encontrado</div>';
        return;
    }
    
    let alertClass = 'alert-info', alertIcon = '💡', shelfLifeMessage = '';
    
    if (product.shelfLife <= 25) {
        alertClass = 'alert-danger'; alertIcon = '⚠️';
        shelfLifeMessage = ' - VIDA ÚTIL CRÍTICA';
    } else if (product.shelfLife <= 75) {
        alertClass = 'alert-warning'; alertIcon = '⚡';
        shelfLifeMessage = ' - Vida útil moderada';
    } else {
        alertClass = 'alert-success'; alertIcon = '✅';
        shelfLifeMessage = ' - Vida útil extendida';
    }
    ...
    productAlert.innerHTML = `
        <div class="product-alert ${alertClass}">
            ...
            Volatilidad: ${product.volatilityMultiplier}x
        </div>
    `;
}
...
function updateSeasonalInfo() {
    ...
    let seasonalMessage = '';
    if (seasonalFactor > 1.3) seasonalMessage = '🔥 TEMPORADA ALTA';
    else if (seasonalFactor > 1.1) seasonalMessage = '📈 Temporada buena';
    else if (seasonalFactor > 0.9) seasonalMessage = '📊 Temporada normal';
    else if (seasonalFactor > 0.6) seasonalMessage = '📉 Temporada baja';
    else seasonalMessage = '❄️ TEMPORADA MUY BAJA';
...
function calculateWithSeasonalNormalization() {
    ...
    if ((month1 > 0 && !month1Type) || (month2 > 0 && !month2Type) || (month3 > 0 && !month3Type)) {
        alert('Por favor selecciona qué mes fue cada dato histórico');
        return;
    }
    ...
    let trend = 'stable', trendMessage = '📊 Demanda Estable';
    if (month1Normalized > 0) {
    ...
    normalizationText += `Promedio normalizado: ${Math.round(normalizedWeightedAverage)} (vs ${Math.round(simpleWeightedAverage)} sin normalizar)`;
    ...
    if (totalDemandNormalized > maxByShelfLife) {
        ...
        shelfLifeDetails.innerHTML = `
            Demanda calculada (${totalDemandNormalized}) excede límite de vida útil.<br>
            Máximo seguro: ${maxByShelfLife} unidades<br>
            Cantidad reducida: ${totalDemandNormalized - maxByShelfLife} unidades
        `;
    } else {
    ...
    finalOrderDescription.textContent = currentStock > 0 ? 
        `Normalización v4.2 - descontando ${currentStock} unidades stock` : 
        'Con normalización estacional v4.2';
    ...
    calculationSteps.innerHTML = `
        <div class="calculation-step">
            <strong>1. Normalización Estacional:</strong><br>
        ...
        <div class="calculation-step">
            <strong>2. Promedio Ponderado:</strong><br>
            (${month1Normalized.toFixed(0)} × 20%) + (${month2Normalized.toFixed(0)} × 30%) + (${month3Normalized.toFixed(0)} × 50%) = ${Math.round(normalizedWeightedAverage)}
        </div>
        <div class="calculation-step">
            <strong>3. Aplicar Factor Estacional:</strong><br>
            ${Math.round(normalizedWeightedAverage)} × ${targetSeasonalFactor.toFixed(3)} = ${normalizedSeasonalDemand} unidades
        </div>
        <div class="calculation-step">
            <strong>4. Stock de Seguridad:</strong><br>
            ${Math.round(normalizedWeightedAverage)} × 30% × ${product.volatilityMultiplier} = ${adjustedStockSafety} unidades
        </div>
        <div class="calculation-step">
            <strong>5. Total + Control Vida Útil:</strong><br>
            ${normalizedSeasonalDemand} + ${adjustedStockSafety} = ${totalDemandNormalized} → Limitado a ${shelfLifeLimited}
        </div>
        <div class="calculation-step">
            <strong>6. Resultado Final:</strong><br>
            ${shelfLifeLimited} - ${currentStock} (stock) = <strong>${finalRecommendation} unidades</strong>
        </div>
    `;
...
</html>iembre': 0.896, 'octubre': 0.725, 'noviembre': 0.548, 'diciembre': 0.180
        }, volatilityMultiplier: 1.8
    },
    'immune-boost-50': {
        name: 'Immune Boost 50ml', shelfLife: 150,
        seasonalIndices: {
    ...
    'skinny-green-480': {
        name: 'Skinny Green 480ml', shelfLife: 75,
        seasonalIndices: {
    ...
    'skinny-green-350': {
        name: 'Skinny Green 350ml', shelfLife: 100,
        seasonalIndices: {
